...
for (const c of cards) {
  ...
  if (startedAt) activeSeconds += Math.max(0, (endAt - startedAt) / 1000);
  let sawGate = false;
...
try {
  ...
  for (const line of lines) {
    ...
    const t = ms(e.ts);
    if (!t) continue;
    if (["quota-pause", "plan-quota-hold"].includes(e.event) && quotaFrom === null) quotaFrom = t;
    if (e.event === "quota-clear" && quotaFrom !== null) {
      const a = Math.max(quotaFrom, from), b = Math.min(t, to);
      if (b > a) quotaSeconds += (b - a) / 1000;
      quotaFrom = null;
    }
...
activeSeconds = Math.max(0, activeSeconds - quotaSeconds);
...
async function scanSession(file) {
  ...
  return belongs ? { file: path.basename(file), sessionId, startedAt, byModel } : null;
}
