...
function attention() {
  ...
  for (const p of (state.plans || [])) {
    ...
    // Ш1.3 · прогон ПРОВАЛЕН — и это самый громкий стоп из всех: PR остался черновиком, в main
    // ветка не уехала, а гейт порядка держит на ней ВСЕ следующие прогоны проекта. До сих пор
    // сюда не попадал вообще (closeState отдаёт «failed» раньше всех веток), поэтому доска
    // рисовала «Никто не ждёт, конвейер идёт сам» над намертво вставшей очередью — 12.08 так
    // простояли три прогона B2-3/4/5 при живом зелёном коде.
    if (st === "failed") {
      const n = (p.result || {}).notice || {};
      rows.push({ k: "merge", ic: "⛔", t: p.goal || "Прогон " + p.id, plan: p.id,
        s: `прогон провален · ${String(n.text || "разбор за человеком").split(".")[0]}`
          + ` — PR черновик, следующие прогоны проекта стоят на этой ветке${held}`, go: "Разобрать" });
    }
